...
public class UI_B : MonoBehaviour
{
    ...
    void Update()
    {
        if (PlayerScript.UI_B_Flag == false) { alfa = 0.0f; }
        if (PlayerScript.UI_B_Flag == true) { alfa = 1.0f; }

        uiImage.position = RectTransformUtility.WorldToScreenPoint(Camera.main, targetObject.position);
        uiImage.position = new Vector3(uiImage.position.x + 0.0f, uiImage.position.y + 50.0f, uiImage.position.z);
